...
public class UserThread extends Thread {
    ...
    public void run() {
        try {
            ...
            if (server.hasUsers()) {
                server.broadcast("REGISTER:Player Two", server.GUEST); 
                player = 2;
            } else {
                server.broadcast("REGISTER:Player One", server.HOST);
            }
            ...
            do {
                ...
                server.broadcast(serverMessage, server.ALL);
 
            } while (!clientMessage.equals("bye"));
 
            socket.close();
 
            server.broadcast("Player " + player + " quitted the server.", server.ALL);
